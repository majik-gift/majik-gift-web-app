...
const EventCard = ({ item, wishlist, wishlists, setWishlists }) => {
    ...
    try {
      ...
      if (wishlist) {
        ...
        const updatedWishlists = wishlists?.filter(
          (wishlist) => data?.response?.details?.event?.id !== wishlist?.id
        );
        // Update wishlist state with filtered products
        setWishlists((prev) => ({
          ...prev,
          events: updatedWishlists,
        }));
      }
    } catch (error) {
      ...
      >
        <Stack gap={1}>
          ...
          <Typography
            ...
            fontFamily={"Lato"}
            fontWeight={"SemiBold"}
          >{`$${item?.total_price ?? "0.00"} `}</Typography>
        </Stack>
